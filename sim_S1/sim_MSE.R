...
for (seed in 1:n.repeats){
  ...
  ## 
  fname = paste0('result/sim_S1/LS-MOFA/', N, '/', seed, '.RData')
  load(fname)
  
  fname = paste0('result/sim_S1/LS-MOFA/', N, '/', seed, '_x.RData')
  load(fname)
  ...
  for (v in 1:V){
    ...
    W_MEFISTO[[v]] = as.matrix(tmp.W[, -ncol(tmp.W)])
  }
...
save(list = c('MSE_all'), file = 'result/sim_S1_MSE.RData')
...
save(list = c('MSE_LSMOFA', 'SE_LSMOFA', 'MSE_MEFISTO', 'SE_MEFISTO', 'p'), 
     file = 'result/sim_S1_MSE_summary.RData')
